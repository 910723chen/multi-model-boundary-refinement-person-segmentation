...
\hline
\texttt{sam\_mask.png} & Stores the initial model-generated mask before final refinement. \\
\hline
\texttt{refined\_mask.png} & Stores the final mask after edge refinement and/or GrabCut. \\
\hline
\texttt{edge\_map.png} & Records the Canny edge map used for boundary comparison. \\
\hline
\texttt{overlay\_comparison.png} & Provides a visual check of the original image, initial mask, and refined result. \\
\hline
\texttt{metrics.txt} & Stores edge-alignment values and optional IoU/Dice values when ground truth is provided. \\
\hline
\texttt{command.json} & Records the version command and parameters for reproducibility. \\
\hline
\end{tabular}
\label{tab:artifacts}
\end{center}
